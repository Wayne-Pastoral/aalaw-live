<aside class="grid-5 sticky" id="aside-blog">    
    ...
    <div>
        ...
        <input type="text" placeholder="Search..." value="<?php echo $searchValue ?>" name="s" id="s" class="banner-text-box" aria-label="Search for..." />
        <input type="submit" id="searchsubmit" value="<?php echo esc_attr_x( 'Search', 'submit button' ); ?>" class="banner-text-btn" aria-label="Submit search" />
    </div>
</form>
      ...
      $cat_args = array(
        ...
        foreach ($categories as $category) {
            ...
            if (!$dui_added && strcasecmp($category->name, 'DUI') > 0) {
                echo '<a href="https://aa.law/category/dui/" title="View all posts in DUI" rel="nofollow" aria-label="View all posts in DUI">DUI</a>';
                $dui_added = true;
            }
            ...
            if ($posts) {
                echo '<a href="' . get_category_link($category->term_id) . '" title="' . sprintf(__('View all posts in %s'), $category->name) . '" rel="nofollow" aria-label="View all posts in ' . $category->name . '">' . $category->name . '</a>';
            }
        }
    ...
    if ($query->have_posts()) :
        ...
        <div class="post__thumbnail">
             <?php echo get_the_post_thumbnail(); ?>
                 ...
                 <div class="blog-item">
                 <?php if (get_locale() == 'en_US') { ?>
        <a class="recent__title" href="<?php the_permalink(); ?>" aria-label="Read more: <?php the_title(); ?>"><?php the_title(); ?></a>
            <span class="date_aside c__lgray"><?= $post_date_formatted ?></span>
        <?php } else { ?>
        <a class="recent__title" href="<?php the_permalink(); ?>" aria-label="Leer más: <?php the_title(); ?>"><?php the_title(); ?></a>
